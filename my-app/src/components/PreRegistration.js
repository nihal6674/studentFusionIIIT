...
const PreRegistration = () => {
  const handleSubmit = () => {
    // Logic for submitting the pre-registration form
    console.log('Submit Pre-Registration');
  };
      ...
      <table className="min-w-full">
        <thead>
          <tr className="bg-[#4866F4] text-white">
            ...
            <th className="p-2">Action</th> {/* New Action Column */}
          </tr>
        </thead>
        <tbody>
          {courses.map(course => (
            <tr key={course.id} className="h-16">
              ...
              <td className="p-2 text-center">
                <button className="bg-[#4866F4] text-white px-4 py-1 rounded">ADD</button> {/* Add button */}
              </td>
            </tr>
          ))}
        </tbody>
